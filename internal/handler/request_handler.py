...
async def book_request(service_request_input: ServiceRequestInput, claims = Depends(require_roles(UserRole.ROLERESIDENT))):
...
async def delete_request(id: Annotated[UUID, Path()], claims = Depends(require_roles(UserRole.ROLERESIDENT))):
...
async def reschedule_request(id: Annotated[UUID, Path()], reschedule_request_input: RescheduleRequestInput, 
                             claims = Depends(require_roles(UserRole.ROLERESIDENT))):
...
@request_router.patch("/service/approve/{id}", dependencies=[Depends(require_roles(UserRole.ROLEADMIN, UserRole.ROLEOFFICER))])
async def approve_request(id: Annotated[UUID, Path()], assigned_to_input: RequestProviderInput):
...
@request_router.patch("/service/complete/{id}", dependencies=[Depends(require_roles(UserRole.ROLEADMIN, UserRole.ROLEOFFICER))])
async def complete_request(id: Annotated[UUID, Path()]):
...
@request_router.get("/service/all", dependencies=[Depends(require_roles(UserRole.ROLEADMIN, UserRole.ROLEOFFICER))])
...
@request_router.get("/service/time-slots", dependencies=[Depends(require_roles(UserRole.ROLERESIDENT))])
